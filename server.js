...
function validateTask(id, body) {
  const b = body || {};
  if (id === 'fundYourWallet') return false; // handled only by the verified funding endpoint
  switch (id) {
...
app.post('/api/signup', async (req,res) => {
  try {
    ...
    user.data.activity.push(activity('Welcome Bouns',50,'Completed','Your individual dashboard is ready.'));
...
app.post('/api/test/verification-code', auth, (req,res) => {
  ...
  console.log(`[VALERIS OTP] user=${req.userId} code=${code} expires=5m`);
  res.json({ ok:true, message:'code generated on server.' });
});
...
app.post('/api/test/fund', auth, async (req,res) => {
  ...
  req.user.data.activity.push(activity('Wallet Funding',amount,'Completed'));
  await save(req); res.json({ ok:true, message:`funding verified. £${amount.toFixed(2)} added.` });
});

function dayKey(date = new Date()) {
  return new Intl.DateTimeFormat('en-CA', {
    timeZone: process.env.TASK_TIMEZONE || 'Africa/Lagos'
  }).format(date);
}

function taskCompletedToday(data, id) {
  const value = data?.dailyTaskCompletions?.[id];
  return Boolean(value && String(value).slice(0, 10) === dayKey());
}

app.post('/api/test/fund-task', auth, async (req,res) => {
  try {
    const amount = Number(req.body?.amount);
    const code = String(req.body?.code || '').trim();
    const task = TASKS.fundYourWallet;

    if (!Number.isFinite(amount) || amount < task.fundingMinimum) {
      return res.status(400).json({
        message:`Fund at least £${task.fundingMinimum.toFixed(2)} to complete this daily task.`
      });
    }
    ...
    req.user.data ??= defaultData();
    req.user.data.dailyTaskCompletions ??= {};

    if (taskCompletedToday(req.user.data, 'fundYourWallet')) {
      return res.status(400).json({ message:'Fund Your Wallet has already been completed today. It is available again at 00:00.' });
    }

    if (!checkCode(req.userId, code)) {
      return res.status(400).json({ message:'Invalid or expired verification code.' });
    }

    // Demo-only wallet funding. No real payment is initiated.
    req.user.data.mainBalance += amount;
    req.user.data.taskBalance += task.reward;
    req.user.data.taskEarnings += task.reward;
    req.user.data.completedTasks += 1;
    req.user.data.dailyTaskCompletions.fundYourWallet = new Date().toISOString();

    req.user.data.activity.push(
      activity('Wallet Funding', amount, 'Completed')
    );
    req.user.data.activity.push(
    ...
    res.status(500).json({ message:'Could not complete the funding task. Please try again.' });
  }
});
...
app.post('/api/test/tasks/:id/submit', auth, async (req,res) => {
  ...
  if (taskCompletedToday(d, id)) return res.status(400).json({ message:'This task has already been rewarded today. It is available again at 00:00.' });
...
app.post('/api/test/withdraw', auth, async (req,res) => {
  ...
  req.user.data ??= defaultData(); settleMaturedInvestments(req.user.data);
  ...
  if (!bank||!recipientName||!iban||!mobile||!email) return res.status(400).json({ message:'Please complete all withdrawal details.' });
  ...
  d.activity.push(Object.assign(activity('Withdrawal',-amount,'Pending','Your withdrawal request is pending processing.'), { withdrawal }));
  await save(req);
  res.json({ ok:true, message:'Your withdrawal request is pending processing.' });
});

function totalAvailableBalance(data) {
  return Number(data?.mainBalance || 0)
    + Number(data?.taskBalance || 0)
    + Number(data?.investmentProfit || 0);
}

function debitTotalAvailableBalance(data, amount) {
  let remaining = Number(amount);
  for (const key of ['mainBalance', 'taskBalance', 'investmentProfit']) {
    const available = Math.max(0, Number(data[key] || 0));
    const take = Math.min(available, remaining);
    data[key] = available - take;
    remaining -= take;
...
app.post('/api/test/invest', auth, async (req,res) => {
  const amount = Number(req.body?.amount);
  req.user.data ??= defaultData();
  const d = req.user.data;
  if (!Number.isFinite(amount) || amount <= 0) return res.status(400).json({ message:'Enter a valid amount.' });

  const availableTotal = totalAvailableBalance(d);
  if (amount > availableTotal) {
    return res.status(400).json({ message:'amount exceeds your available balance.' });
  }

  // Demo-only: use funds available across the member's total available balance.
  // Active investment capital remains locked and cannot be invested a second time.
  if (!debitTotalAvailableBalance(d, amount)) {
    return res.status(400).json({ message:'Unable to allocate the amount from your available balance.' });
  }

  const profit = Math.round(amount * 0.30 * 100) / 100;
  const startedAt = new Date().toISOString();
  const maturesAt = new Date(Date.now() + 24*60*60*1000).toISOString();
  const inv = { id:crypto.randomUUID(), amount, profit, startedAt, maturesAt, status:'active' };
  d.investmentBalance += amount;
  d.investments.push(inv);
  d.activity.push(activity('Investment Started',-amount,'Active','30% return scheduled after 24 hours.'));
  await save(req);
  res.json({ ok:true, investment:inv, message:'investment started.' });
});
...
async function start() {
  try {
    ...
    app.listen(PORT, () => console.log(`ValerisBank running on ${PORT}`));
  } catch (error) {
