...
app.post('/api/test/withdraw', auth, async (req,res) => {
  ...
  d.activity.push(Object.assign(activity('Demo Withdrawal',-amount,'Pending','Your withdrawal request is pending processing.'), { withdrawal }));
  await save(req);
...
app.post('/api/test/invest', auth, async (req,res) => {
  ...
  if (amount > availableTotal) {
    return res.status(400).json({ message:'Simulation amount exceeds your total available balance.' });
  }
  ...
  if (!debitTotalAvailableBalance(d, amount)) {
    return res.status(400).json({ message:'Unable to allocate the simulation amount from your available balance.' });
  }
  ...
  await save(req);
  res.json({ ok:true, investment:inv, message:'Demo investment started using your total available balance. Capital plus a 30% simulated profit will mature after 24 hours.' });
});
